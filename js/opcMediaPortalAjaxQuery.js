il.opcMediaPortalAjaxQuery = (function (scope) {
  ...
  pub.addAction = function () {
    ...
    $('#OpencastSelectionModal .ilTableNav')
      .hide();
    pro.addTitle();
  ...
  pub.openSelectionModal = function (is_static) {
    ...
    pro.displayWaitBox();
    let url = document.querySelector('#iv-opc-url')
      .getAttribute('data-iv-opencast-url');
    window.location.replace(url);
  };

  pro.displayWaitBox = function () {
    if ($('body')
      .find('.openCastWaitBox').length === 0) {
      $('body')
        .append('<div class="openCastWaitBox"></div>');
    }
    $('.openCastWaitBox')
      .html('<div class="col-xs-12 ffmpeg_spinner" style="width: 100%;height: 100%;position: absolute;left: 40%;top: 40%;"><img src="Customizing/global/plugins/Services/Repository/RepositoryObject/InteractiveVideo/templates/images/spinner.svg"/></div>');
  };
  ...
  pro.addTitle = function () {
    let title = $('#opc_url')
      .val();
    $('.opc_selected_title')
      .html(il.Language.txt('title') + ': ' + title);
  };

  pro.parseEventId = function (that) {
    let url = that.find('.il-item-title a')
      .attr('href');
    let event_id = url.split('xvid_id_url=');
    let title = that.find('.il-item-title a')
      .html();

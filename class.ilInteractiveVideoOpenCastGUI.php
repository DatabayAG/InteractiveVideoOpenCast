...
class ilInteractiveVideoOpenCastGUI implements ilInteractiveVideoSourceGUI
{
    ...
    public function getForm($option, $obj_id) : ilRadioOption
    {
        ...
        $this->addConfigStructure();
        $this->ajax_url = $this->dic->ctrl()->getLinkTargetByClass(['ilRepositoryGUI', 'ilObjInteractiveVideoGUI'],
            'addVideoSelectionForm', '', false, false);
        $current_url = new URI(ILIAS_HTTP_PATH . '/' . $this->dic->ctrl()->getLinkTargetByClass([ilObjPluginDispatchGUI::class,
    ...
    {
        global $DIC;
        $DIC->ctrl()->setParameter(new ilObjInteractiveVideoGUI(), 'xvid_plugin_ctrl',
            ilInteractiveVideoOpenCastGUI::class);
    ...
    private function addTab(?ilObjInteractiveVideo $obj, bool $active = false)
    {
        ...
        if ($active === true || ($obj !== null && $obj->getSourceId() === 'opc')) {
            ...
            $DIC->tabs()->addSubTab(self::CUSTOM_SOURCE_TAB_ID,
                ilInteractiveVideoPlugin::getInstance()->txt('opc_video'), $current_url);
